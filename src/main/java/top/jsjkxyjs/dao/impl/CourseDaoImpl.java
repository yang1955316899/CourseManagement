...
public class CourseDaoImpl extends BaseDao implements CourseDao {
	...
	public List<Course> getChooseCourses() {
		...
		conn = getConnection();
		String sql = "select t_course.*,count from (select count(StudentId) count,id from (select * from (select id from t_course where Choose = 1) t_id left join t_sc on t_id.id = t_sc.CourseId) t_count group by id) t_count,t_course where t_count.id = t_course.id and count < MaxSize";
		try {
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
			while (rs.next()) {
