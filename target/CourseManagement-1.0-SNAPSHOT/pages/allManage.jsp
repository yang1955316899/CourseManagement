...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#currentTableId',
            url: '../SuperManageServlet?action=getAllUser',  //!!!!表格数据接口
            toolbar: '#toolbarDemo',
            cols: [[
        ...
        form.on('submit(data-search-btn)', function (data) {
            ...
            let userId = target.userId; //用户输入的id
            let userName = target.userName; //用户输入的name

            let reg = /^[0-9]*$/;
            if (!reg.test(userId)) {
                layer.msg("用户编号应为十位纯数字");
            } else {
                $.getJSON("../SuperManageServlet?action=superSearch", target, function (data) {
                    //表格重载
                    table.reload('currentTableId', {
                        url: '../SuperManageServlet?action=superSearch&userId=' + userId + "&userName=" + userName
                        , where: {} //设定异步数据接口的额外参数
                    });
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {
             ...
             * */
                ...
                $.getJSON("../CounselorServlet?action=delUserList&a=" + a, function (data) {
                    ...
                    table.reload('currentTableId', {
                        /***!!!!!!!数据表格的重载!!!!!！****/
                        url: '../SuperManageServlet?action=getAllUser' //数据接口
                        , where: {} //设定异步数据接口的额外参数
                        //,height: 300
                    });
                    if (data.result !== 0) {
